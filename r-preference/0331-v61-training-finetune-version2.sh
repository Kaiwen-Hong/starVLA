#!/usr/bin/env bash
# ============================================================
# v61 OFT finetune v2 — Training only (data must be ready)
#
...
#
# Data: place_cup5_tray5 (clean1, wp4) + place_stapler_stand (clean1)
...
#
# Usage:
#   bash r-preference/0331-v61-training-finetune-version2.sh 2>&1 | tee v61_finetune_v2.log
# ============================================================
...
LEROBOT_REPO="${LAB_ROOT}/.cache/huggingface/lerobot/custom_v0320_v61_repo"

SPLIT_TASKS=(
  place_cup5_tray5_clean1
  place_cup5_tray5_wp4
...
echo "Pipeline:  v61 OFT finetune v2 (training only)"
echo "============================================"
...
if [ ! -f "${LEROBOT_REPO}/meta/info.json" ]; then
  ...
  echo "Run 0331-v626151-data-processing.sh first!"
  exit 1
fi
