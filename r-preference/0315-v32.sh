#!/usr/bin/env bash
# ============================================================
# v32-ee 训练脚本（交互式 GPU 节点直接运行）
#
# v32 = 3 variants: place_container_plate (clean1, wp5) + place_object_stand (clean1)
# 300 episodes, 16D EE space
#
# 前提：已在 4xH100/H200 计算节点上
# 用法：bash r-preference/0315-v32.sh
# ============================================================
set +u
...
accelerate launch \
  ...
  --datasets.vla_data.data_root_dir playground/Datasets/CustomEE_v32 \
  --datasets.vla_data.data_mix custom_v0309_v32_ee \
  ...
  --run_id v0309_v32_ee_qwenPI_requeue \
  --wandb_project starVLA_v32 \
  --wandb_entity hca
