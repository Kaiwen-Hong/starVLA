...
To tackle the chanllenge, we propose \textbf{Self-Labeled Preference Transfer (SPT)}.
...
Across five preference axes in simulation, the source model labels unlabeled target demonstrations correctly \textbf{[81]}\% of the time, well above the 50\% chance level, and SPT improves preference-following by \textbf{[20]} points over a finetune that does not self-label, with task success unchanged.
On a UR5e, SPT follows the commanded preference on two real target tasks that carry no preference labels, with preference-following transferred from a related source: pushing from pick-and-place, and billiards from air hockey.
...
\textcolor{red}{I did not change this part}
We evaluate SPT in both simulation and the real world. Because existing simulation-based manipulation benchmarks vary appearances and initializations but demonstrate a single behavior per task~\citep{libero, liberoplus}, we synthesize preference-varied demonstrations on RoboTwin~\citep{robotwin} across five axes: grasp contact height, release height, obstacle clearance, grasp orientation, and placement location. In the real world, we test with a real UR5e under two cross-task transfer settings: air-hockey to billiards, pick-and-place to pushing.
...
(4) Across simulation and a real UR5e, SPT achieves preference control on unlabeled tasks (81\% recognition accuracy, +20 points in preference-following over baselines), and we analyze when the transfer holds.
...
We test on a UR5e under two cross-task settings. For \emph{obstacle clearance}, we label a near vs.\ far clearance preference on pick-and-place and transfer it to pushing; for \emph{strike strategy}, we label a direct vs.\ bank-shot preference on air hockey and transfer it to billiards. As in simulation, the preference is left out of the new task's instruction, and each new task shares the source axis but differs in objects, dynamics, and goal. We defer the task and scene details to the appendix.
...
% We construct the ablation variants by differently ablating the two components in SPT: Stage-A prediction loss, and Stage-B self-label conditioning.
% \begin{itemize}
%     \item \textbf{Naive Fine-tuning.} Naively fine-tune all the training pairs with action loss.
%     \item \textbf{No Recognition Loss.} Do self-label conditioning without the prediction loss. 
%     \item \textbf{No Self-labeling.} Fine-tune the prediction-trained model without the self-labels.
% \end{itemize}
...
target, per axis. It is well above chance on all five axes (96.4\% on average,
and at least 90\% on every axis), so the preferences learned on the source carry
over to the target's new tasks and objects with no preference stated in the
instruction, and the self-labels that Stage~B trains on are therefore nearly
noise-free. The two lowest, placement location (90\%) and obstacle clearance
(92\%), are the more relational axes, where the preference shows up in the
object's final location or the detour path rather than in a single arm pose.
These predictions are the labels Stage~B trains on.
...
and Table~\ref{tab:real_main} on the UR5e. SPT follows the commanded preference
across the simulated axes and on both real task pairs. The contrast with the
baseline is sharpest on the real robot, where Naive FT follows the prompt in only
4--5 of 10 rollouts, no better than chance, yet still completes the task in 8--10.
Finetuning on the unlabeled target without self-labeling thus yields a competent
but single-minded policy that is insensitive to the preference prompt, exactly the
failure SPT is designed to remove: SPT lifts following to 9--10 of 10 without
giving up task success. 
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    Release height       & -- & 81 & -- & 91 \\
    Obstacle clearance   & -- & 75 & -- & 82 \\
    Grasp orientation    & -- & 83 & -- & 92 \\
    Placement location   & -- & 95 & -- & 86 \\
    \hline
    Average              & -- & \textbf{--} & -- & -- \\
    \hline
  \end{tabular}
...
\begin{table}[t]
  ...
  \begin{tabular}{llccccc}
    ...
    \hline
    Obstacle clearance & near    & 4/10 & 10/10 & 9/10 & 10/10 \\
    (push)             & far     & 5/10 & 10/10 & 10/10 & 10/10 \\
    Strike strategy    & direct  & 5/10 & 9/10 & 8/10 & 8/10 \\
    (billiards)        & bank    & 5/10 & 10/10 & 9/10 & 10/10 \\
    \hline
  \end{tabular}
\end{table}
...
SPT has two ingredients: the Stage-A recognition loss, which co-trains preference
recognition with the policy over the shared backbone, and the Stage-B self-label
conditioning, which trains the action expert on the self-inferred preference of
each target demonstration. We turn each on and off independently, giving a
$2\times2$ over the two factors (Table~\ref{tab:ablation}), averaged over the five
simulated axes. \textbf{Naive FT} uses neither (Section~\ref{sec:setup}).
\textbf{Recog.\ only} adds the Stage-A recognition loss but finetunes Stage~B on
the target without conditioning on the self-labels. \textbf{Self-label only}
conditions Stage~B on the self-labels but starts from a policy whose backbone was
not co-trained with the recognition loss. \textbf{SPT} uses both. Comparing
Recog.\ only with SPT isolates the effect of conditioning on the self-labels, and
comparing Self-label only with SPT isolates the effect of the Stage-A recognition
loss beyond merely supplying those labels.

\textcolor{red}{TODO: analysis once the ablation results are in.}

\begin{table}[t]
  \centering
  \small
  \caption{\textbf{Ablation of SPT's two ingredients} (simulation, averaged over
  the five axes). \emph{Stage-A recog.}\ is the recognition loss co-trained with
  the policy over the shared backbone; \emph{Stage-B self-label} conditions the
  target finetune on the self-inferred preference.}
  \label{tab:ablation}
  \begin{tabular}{lcccc}
    \hline
    Configuration & Stage-A recog. & Stage-B self-label & Following Acc.\ (\%) & Success (\%) \\
    \hline
    Naive FT        & $\times$    & $\times$    & -- & -- \\
    Recog.\ only    & \checkmark  & $\times$    & -- & -- \\
    Self-label only & $\times$    & \checkmark  & -- & -- \\
    SPT             & \checkmark  & \checkmark  & \textbf{--} & \textbf{--} \\
    \hline
  \end{tabular}
\end{table}
