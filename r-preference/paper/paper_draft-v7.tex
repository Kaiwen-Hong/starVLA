...
Across five preference axes in simulation, the source model labels the unlabeled target with \textbf{96.4\%} accuracy, and SPT improves preference-following by \textbf{25.4\%} over a naive finetune (FT) baseline that does not self-label.
On a real UR5e, SPT transfers preference control to two unlabeled target tasks, pushing and billiards, raising preference-following by about \textbf{40} \% over the naive FT baseline while keeping task success high.
...
\keywords{Human-Robot Interaction and Natural Language Instruction, Imitation Learning}


\section{Introduction}

Robot datasets today span a remarkable range of tasks, scenes, and objects, enabling significant advances in generalist robot policies that condition on language and generalize across embodiments and environments~\cite{o2024open, liberoplus, libero, nasiriany2024robocasa, mees2022calvin}. Nevertheless, these datasets typically demonstrate only one mode to succeed for each task ~\citep{libero, liberoplus}.
...
Across these settings the source-trained recognizer recovers the correct preference on the unlabeled target \textbf{96.4}\% of the time, preference-following improves by 25.4\% over a finetune baseline without self-labeling, and on the UR5e SPT follows the commanded preference on both unlabeled target tasks.
...
(4) Across simulation and a real UR5e, SPT achieves preference control on unlabeled tasks, with 96.4\% recognition accuracy and a 25 \% gain in preference-following over the no-self-label baseline.
...
\begin{figure}[t]
  ...
  \caption{\textbf{Self-Labeled Preference Transfer (SPT).} During Stage A, a shared VLM backbone is jointly trained on a preference-labeled source dataset for two objectives: (i) preference-conditioned action prediction through the action expert, and (ii) preference recognition through a language-model head that answers preference-related questions from demonstration trajectories and proprioceptive observations. The resulting backbone therefore learns both how to execute and how to recognize preferences. During Stage B, the preference recognizer is frozen and used to self-label unlabeled target demonstrations with preference pseudo-labels. These predicted preferences are then appended to the task prompt to adapt the action policy on the target task without any preference annotations. At deployment, users specify a preference prompt, and the adapted policy generates preference-consistent behaviors on unseen tasks and objects.}
  \label{fig:method}
\end{figure}
...
With experiments in simulation and on a real robot, we aim to answer two questions:
...
Each target shares the source axis but differs in objects, dynamics, and goal.

\textbf{Metrics.}
To evaluate the performance of label-free preference transfer, we consider 3 metrics:
\begin{itemize}
    \item \textbf{Recognition Accuracy:} How often the frozen source predictor assigns the correct preference to a target demonstration compared with random guess (50\%).
    \item \textbf{Preference-following Accuracy:} The fraction of rollouts whose behavior matches the commanded preference, scored by a per-axis statistic $\phi$, with continuous axes thresholded from source statistics and categorical axes matched directly.
    \item \textbf{Task Success:} The completion rate regardless of preference-following accuracy.
\end{itemize}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    \hline
    Average              & 59.4 & \textbf{84.8} & 88 & \textbf{89.2} \\
    \hline
  \end{tabular}
\end{table}
...
robot, which is barely above the 50\% chance line, yet still completes the task at
...
preference. Our SPT removes this failure. It lifts following to 84.8\% in
...
conditions), rising on three simulated axes and decreasing by at most three points on
...
In this work, we introduced label-free preference transfer, a new problem setting in which preferences are densely labeled on a source task set but unavailable on target tasks. Rather than treating preference annotation as a recurring requirement for every new task, we ask whether preference-following itself can become a transferable capability of vision-language-action models. To address this challenge, we proposed Self-Labeled Preference Transfer (SPT), a two-stage framework that jointly learns preference-conditioned action generation and preference recognition on labeled source tasks, then reuses the learned recognizer to automatically infer preference labels for unlabeled target demonstrations. By converting unlabeled target data into preference-conditioned training signals, SPT preserves and transfers preference steerability during adaptation. Experiments across five preference axes in simulation and two cross-task real-robot transfer settings demonstrate that preferences can successfully transfer beyond the tasks on which they were originally labeled. The source-trained recognizer achieves 96.4\% accuracy on unlabeled target demonstrations, while SPT improves preference-following accuracy by 25.4\% over a naive finetuning baseline without sacrificing task success. On a real UR5e, SPT transfers preference control to previously unseen pushing and billiards tasks, increasing preference-following from near-chance performance to 92.5\%. These results suggest that execution preferences can be learned once and reused across tasks, much like the task, scene, and object generalization capabilities of modern VLAs. More broadly, our work takes a step toward robot policies that are not only capable of understanding what users want to accomplish, but also how they prefer those tasks to be carried out.
