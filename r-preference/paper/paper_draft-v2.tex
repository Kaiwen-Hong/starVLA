...
\begin{abstract}
Robot datasets span many tasks, scenes, and objects, but usually demonstrate a single way to do each task.
The same instruction, however, often admits several acceptable executions that differ in \emph{how} the task is carried out rather than \emph{what} is achieved; we call these axes \emph{preferences} (for example, grasping an object low or high, or releasing it gently or from above).
Teaching a policy to follow such preferences normally means collecting demonstrations and labeling each with its preference, and this labeling recurs for every new task.
We ask whether preference-following can instead be learned once and transferred: preferences are densely labeled on a source task set, while a target task set provides demonstrations whose instructions do not state the preference.
A naive finetune on the target ties no motion to a preference, and the steerability from source training washes out.
We propose \textbf{Self-Labeled Preference Transfer (SPT)}.
...
Across five preference axes in simulation, the source model labels the unlabeled target with \textbf{[81]}\% accuracy (chance \textbf{[50]}\%) and SPT improves preference-following by \textbf{[20]} points over a finetune baseline that does not self-label, with task success unchanged.
On a UR5e, SPT follows the commanded preference on two task sets it never saw labeled: pick-and-place to pushing, and air hockey to billiards.
...
\begin{figure}[t]
  ...
  \caption{\textbf{Motivation and overview of SPT.} One single task can often be completed in multiple valid ways that reflect different user preferences. Existing robot datasets and VLAs primarily learn \emph{what} to accomplish, but not \emph{how} to execute tasks according to user preferences. We propose \textbf{Self-Labeled Preference Transfer (SPT)}, which learns preference-following from densely labeled source tasks and transfers this capability to unseen tasks and objects without requiring preference labels on the target task.}
  \label{fig:teaser}
\end{figure}
...
(1) We identify and analyze the missing axes, preference following in multi-solution tasks, in popular robotic datasets.
(2) We introduce a new problem in training VLA to understand and follow preferences, \emph{label-free preference transfer}, where preferences are densely labeled on source tasks, and must be inherited on target tasks without preference labels. To our knowledge, this is the first work to treat execution preference as a cross-task transfer capability of VLAs. (3) We propose SPT, a two-stage method that co-trains a preference-conditioned action head and a preference-recognition head over a shared vision-language backbone, then repurposes the source-trained backbone to self-label the unlabeled target demonstrations, supplying the missing preference labels without any target-side annotation.
(4) We demonstrate through simulation and real-robot experiments that SPT enables effective preference control on unlabeled tasks, achieving 81\% recognition accuracy and 20-point improvements in preference-following over baselines, and we analyze when this transfer succeeds.
...
\textbf{Architecture.}
  \textcolor{red}{SPT is a single vision--language--action model with a shared backbone and two
...
action expert at a lower learning rate. 
