...
conditions on the previous chunk's un-executed actions as a known prefix, which
is what powers the `ur5n/` real-world deployment.

## What's added

| Component | Path |
|---|---|
| Framework | `starVLA/model/framework/QwenDiscreteDiffusion.py` |
...
| Real-world deployment scripts | `ur5n/dd/`, `ur5n/2dd/` (discrete diffusion) and `ur5n/fm/`, `ur5n/2fm/` (flow-matching baseline) |
...
## Real-world deployment (`ur5n/`)

Two task variants (one per checkpoint) live alongside their flow-matching
counterparts so the two heads can be benchmarked head-to-head:

| Dir | Modality | Task |
|---|---|---|
| `ur5n/dd/` | Discrete diffusion | Pick from a static pan, place on a rotating turntable. |
| `ur5n/2dd/` | Discrete diffusion | Pick from a rotating turntable, place on a static pan. |
| `ur5n/fm/` | Flow-matching (QwenPI) | Same task as `dd/`. |
| `ur5n/2fm/` | Flow-matching (QwenPI) | Same task as `2dd/`. |

### Server / client split

To avoid the ~30 s checkpoint reload between code edits, the policy lives in a
long-running `inference_server.py` that listens on a Unix socket, and the
control loop talks to it through `RemoteModel` (`remote_model.py`). The two
modalities use different sockets so an FM server and a DD server can run
side-by-side.

```bash
# Terminal 1 — start the inference server (loads once, stays warm)
python ur5n/dd/inference_server.py --checkpoint <path> --socket /tmp/starvla_infer_dd.sock

# Terminal 2 — run the closed-loop controller (defaults to --use_server)
python ur5n/dd/closedloop_rtc_v6.py        # interactive episode loop
python ur5n/2dd/closedloop_rtc_v7.py       # single-shot live (turntable variant)
python ur5n/dd/closedloop_sync.py          # blocking sync baseline (no RTC)
```

Key flags on the closed-loop scripts (see `argparse` block for the full list):

```
--n_actions / --inference_delay   chunk size and prefix carry-over
--decode_temperature / --choice_temperature   MaskGIT sampling
--use_simple_max                  single-forward argmax decode
--fixed_steps / --hard_mask / --early_stop    pass-through to predict_action_realtime
--use_server / --no_use_server    use an external inference server
--server_socket <path>            override default socket
```

### Open-loop offline eval

`ur5n/dd/openloop_eval.py` and `ur5n/fm/openloop_eval.py` run
`predict_action` over a held-out dataset and report MSE / L1 (overall, per-dim,
per-step). Use these to compare new checkpoints without touching the robot.
...
└── discrete_diffusion/
    ...
    ├── models.py                            # standalone DiscreteDiT (unused by Layerwise head)
    └── __init__.py
ur5n/
├── dd/   2dd/                               # discrete-diffusion deployment
└── fm/   2fm/                               # flow-matching baseline (QwenPI)
```
