#!/usr/bin/env bash
# ============================================================
# Pref-VLA Stage B MAIN — height (place_playingcards1_box, 100 ep)
#   Warm-start from VQA-Stage-A 35k.
#   Pseudo-labels from r-preference/eval/pref_pseudo_labels_height_B.json
...
DATA_ROOT=""
for c in /mnt/localssd/kaiwenh/pref/data/height/taskB /mnt/localssd/kevin/pref/data/height/taskB; do
  [ -d "$c" ] && DATA_ROOT="$c" && break
done
[ -z "$DATA_ROOT" ] && { echo "ERROR: pref/data/contact/taskB not found"; exit 1; }

CACHE_PATH="r-preference/eval/pref_pseudo_labels_height_B.json"
...
echo "============================================"
echo "Run:           pref_main_stage_b_v1_height"
echo "Node:          $(hostname) ($(whoami))"
echo "DATA_ROOT:     $DATA_ROOT"
echo "PSEUDO CACHE:  $CACHE_PATH"
echo "WARM-START:    VQA-Stage-A 25k (height)"
...
accelerate launch \
  ...
  --config_yaml examples/preference/train_files/starvla_pref_stage_b_main_height.yaml \
