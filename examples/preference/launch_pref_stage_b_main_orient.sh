#!/usr/bin/env bash
# ============================================================
# Pref-VLA Stage B MAIN — orient (move_can5_away, 100 ep)
#   Warm-start from VQA-Stage-A 35k.
#   Pseudo-labels from r-preference/eval/pref_pseudo_labels_orient_B.json
...
DATA_ROOT=""
for c in /mnt/localssd/kaiwenh/pref/data/orient/taskB /mnt/localssd/kevin/pref/data/orient/taskB; do
  [ -d "$c" ] && DATA_ROOT="$c" && break
done
[ -z "$DATA_ROOT" ] && { echo "ERROR: pref/data/contact/taskB not found"; exit 1; }

CACHE_PATH="r-preference/eval/pref_pseudo_labels_orient_B.json"
...
echo "============================================"
echo "Run:           pref_main_stage_b_v1_orient"
echo "Node:          $(hostname) ($(whoami))"
echo "DATA_ROOT:     $DATA_ROOT"
echo "PSEUDO CACHE:  $CACHE_PATH"
echo "WARM-START:    VQA-Stage-A 25k (orient)"
...
accelerate launch \
  ...
  --config_yaml examples/preference/train_files/starvla_pref_stage_b_main_orient.yaml \
