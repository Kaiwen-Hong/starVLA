#!/usr/bin/env bash
# ============================================================
# Pref-VLA Stage B MAIN — place (place_soap2_stand, 100 ep)
#   Warm-start from VQA-Stage-A 35k.
#   Pseudo-labels from r-preference/eval/pref_pseudo_labels_place_B.json
...
DATA_ROOT=""
for c in /mnt/localssd/kaiwenh/pref/data/place/taskB /mnt/localssd/kevin/pref/data/place/taskB; do
  [ -d "$c" ] && DATA_ROOT="$c" && break
done
[ -z "$DATA_ROOT" ] && { echo "ERROR: pref/data/contact/taskB not found"; exit 1; }

CACHE_PATH="r-preference/eval/pref_pseudo_labels_place_B.json"
...
echo "============================================"
echo "Run:           pref_main_stage_b_v1_place"
echo "Node:          $(hostname) ($(whoami))"
echo "DATA_ROOT:     $DATA_ROOT"
echo "PSEUDO CACHE:  $CACHE_PATH"
echo "WARM-START:    VQA-Stage-A 25k (place)"
...
accelerate launch \
  ...
  --config_yaml examples/preference/train_files/starvla_pref_stage_b_main_place.yaml \
