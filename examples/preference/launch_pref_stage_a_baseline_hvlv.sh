#!/usr/bin/env bash
# ============================================================
# Pref-VLA Stage A baseline (no VQA) — HVLV category
#   Data:    /mnt/localssd/$USER/pref/data/hvlv
#   Spec:    25k steps, save every 5k, eff batch 64
#   Run on:  H100 (per user plan 2026-05-23)
#
# Usage:
#   bash examples/preference/launch_pref_stage_a_baseline_hvlv.sh
#   MAX_STEPS=200 NO_SAVE=1 bash examples/preference/launch_pref_stage_a_baseline_hvlv.sh   # smoke
# ============================================================
set +u
...
DATA_ROOT=""
for c in /mnt/localssd/kaiwenh/pref/data/hvlv /mnt/localssd/kevin/pref/data/hvlv; do
  [ -d "$c" ] && DATA_ROOT="$c" && break
done
[ -z "$DATA_ROOT" ] && { echo "ERROR: pref/data/hvlv not found"; exit 1; }
...
echo "============================================"
echo "Run:         pref_baseline_stage_a_v1_noVQA_hvlv"
...
accelerate launch \
  ...
  --config_yaml examples/preference/train_files/starvla_pref_stage_a_baseline_hvlv.yaml \
