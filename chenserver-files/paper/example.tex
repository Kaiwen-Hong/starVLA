...
For the \textbf{discrete diffusion policy}, actions are discretized into 512 bins via uniform quantization.
The input is the observation concatenated with bin embeddings (dim 128) of the partially masked token sequence.
...
A learned null vector replaces the time conditioning.
...
For ContinuousRTC, $\Pi$GDM guidance with exponential-decay soft masking and clipping threshold $\beta{=}5.0$ steers generation toward the frozen prefix.
For DiscreteRTC, the frozen prefix tokens are directly placed, and early stopping naturally reduces the number of unmasking steps.
...
Demonstrations are recorded at 15\,Hz via the FastUMI pipeline.
...
Training uses 8 GPUs with DeepSpeed ZeRO-2 and gradient checkpointing; deployment runs on a single RTX 4090.

\textbf{Policy Architecture.}
All policies use Qwen2.5-VL-3B-Instruct~\citep{yang2025qwen3} as the VLM backbone.
Both action heads share the same QwenPI-style layerwise cross-attention DiT~\citep{peebles2023scalable}: 36 blocks (matching the VLM), each with AdaLN, cross-attention to the corresponding VLM hidden states, self-attention, and a feedforward network.
...
For the \textbf{flow-matching head}, a three-layer MLP fuses per-timestep action embeddings with sinusoidal flow-time embeddings (output dim 2048); a sinusoidal \texttt{TimestepEncoder} provides AdaLN conditioning; and a two-layer MLP (hidden dim 1024) decodes the velocity output.

For the \textbf{discrete diffusion head}, actions are quantized into 256 bins per dimension, yielding $16 \times 10 = 160$ tokens per chunk.
Tokens (including a learnable \texttt{[MASK]} at index 256) are embedded via \texttt{nn.Embedding}(257, 2048).
No timestep conditioning is used.
...
The flow-matching head uses 4 Euler denoising steps; the discrete diffusion head uses 8 MaskGIT unmasking steps with a cosine decode schedule (decode temperature 0.0, choice temperature 0.1).
For DiscreteRTC, when a previous chunk is available, its shifted suffix serves as the inpainting prefix ($d$ timesteps pre-filled); remaining positions start fully masked.
By default we use fixed unmasking steps; optionally steps scale as $\max(1, \lfloor 8 \cdot d / H \rfloor)$.
...
RTC guidance & $\Pi$GDM ($\beta{=}5.0$, exp.\ decay) & Early stopping \\
...
\textbf{Hyperparameter} & \textbf{Flow-Matching} & \textbf{Discrete Diffusion} \\
\midrule
\multicolumn{3}{l}{\textit{VLM Backbone \& Action Head (Layerwise Cross-Attention DiT)}} \\
VLM backbone & \multicolumn{2}{c}{Qwen2.5-VL-3B-Instruct (2048 dim, 36 layers)} \\
DiT layers / heads / head dim & \multicolumn{2}{c}{36 / 32 / 64 (inner dim 2048)} \\
Dropout / future vision tokens & \multicolumn{2}{c}{0.2 / 32} \\
Action encoder & 3-layer MLP + sinusoidal time & nn.Embedding(257, 2048) \\
...
Representation & Continuous & 256-bin quantization (160 tokens) \\
\midrule
\multicolumn{3}{l}{\textit{Training}} \\
