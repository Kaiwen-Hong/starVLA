...
class ActionBinning(nn.Module):
    ...
    def __init__(
        ...
        if representation == "bit":
            ...
            assert num_bins == expected_bins, (
                f"bit representation requires num_bins={expected_bins}, got {num_bins}"
            )
    ...
    def decode_logits(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        if self.representation == "bin":
            indices, _ = self._logits_to_indices_bin(
    ...
    def sample_indices_from_logits(
        ...
        return self._logits_to_indices_bit(
            logits, deterministic=deterministic, generator=generator
        )
