...
Usage:
    python ur5-dd/step5-live-inference-viz.py
    python ur5-dd/step5-live-inference-viz.py --arm left --use_simple_max
    python ur5-dd/step5-live-inference-viz.py --decode_temperature 0.5
"""
...
def visualize(world_poses, camera_image, current_ee, save_path):
    ...
    fig.suptitle(
        f'Step 5 (Discrete Diffusion): Live Robot State + Predicted Trajectory\n"{INSTRUCTION}"',
...
def main():
    parser = argparse.ArgumentParser(
        description="Full inference with live robot state, viz only (Discrete Diffusion)")
    parser.add_argument(
        "--checkpoint", type=str,
    ...
    try:
        while True:
            ...
            save_path = Path("ur5-dd") / f"step5_live_inference_viz_{timestamp}.png"
