#!/usr/bin/env bash
# ============================================================
# v41 OFT finetune v2 — Training only (data must be ready)
#
...
#
# Data: place_cup_tray (clean1, wp4) + place_stapler_stand (clean1)
#       3 variants, 250 episodes each, 750 total
#
# Prerequisite: run 0331-v4151-data-processing.sh first!
#
# Prerequisites:
...
#
# Usage:
#   bash r-preference/0331-v41-training-finetune-version2.sh 2>&1 | tee v41_finetune_v2.log
# ============================================================
...
LEROBOT_REPO="${LAB_ROOT}/.cache/huggingface/lerobot/custom_v0320_v41_repo"
...
DATA_MIX="custom_v0320_v41"
RUN_ID="v0320_v41_qwenOFT_finetune_v2"
WANDB_PROJECT="starVLA_v41_finetune_v2"
...
echo "Pipeline:  v41 OFT finetune v2 (training only)"
echo "============================================"
...
if [ ! -f "${LEROBOT_REPO}/meta/info.json" ]; then
  ...
  echo "Run 0331-v4151-data-processing.sh first!"
  exit 1
fi
