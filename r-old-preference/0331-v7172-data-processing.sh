#!/usr/bin/env bash
# ============================================================
# Data processing for v71 + v72 (joint-space, 14D)
#
# Processes both datasets SEQUENTIALLY to avoid symlink race
# conditions on the shared place_stapler_stand variant.
#
# v71: place_cup1_tray5 (clean1, wp4) + place_stapler_stand (clean1) = 750 ep
# v72: place_cup1_tray5 (clean1, wp5) + place_stapler_stand (clean1) = 750 ep
#
# Steps per dataset:
...
#   bash r-preference/0331-v71-training-finetune-version2.sh
#   bash r-preference/0331-v72-training-finetune-version2.sh
#
# Prerequisites:
...
#
# Usage:
#   bash r-preference/0331-v7172-data-processing.sh 2>&1 | tee v7172_data.log
# ============================================================
...
echo "============================================"
echo "Node:      $(hostname)"
echo "Pipeline:  v71 + v72 data processing (sequential)"
echo "============================================"

cd "${KEMPNER_REPO}/policy/pi05"

# ============================================================================
# v71: 3 variants, 750 episodes
# ============================================================================
STEP_LOG "v71: Processing raw -> HDF5 (joint-space)"

V71_VARIANTS=(
  "place_cup1_tray5:clean1"
  "place_cup1_tray5:wp4"
  "place_stapler_stand:clean1"
)
V71_TDATA="training_data/custom_v0320_v71"
V71_REPO="custom_v0320_v71_repo"

process_dataset "v71" "$V71_TDATA" "${V71_VARIANTS[@]}"

STEP_LOG "v71: Generating LeRobot dataset"

generate_lerobot "v71" "$V71_TDATA" "$V71_REPO" 750

# ============================================================================
# v72: 3 variants, 750 episodes
# ============================================================================
STEP_LOG "v72: Processing raw -> HDF5 (joint-space)"

V72_VARIANTS=(
  "place_cup1_tray5:clean1"
  "place_cup1_tray5:wp5"
  "place_stapler_stand:clean1"
)
V72_TDATA="training_data/custom_v0320_v72"
V72_REPO="custom_v0320_v72_repo"

process_dataset "v72" "$V72_TDATA" "${V72_VARIANTS[@]}"

STEP_LOG "v72: Generating LeRobot dataset"

generate_lerobot "v72" "$V72_TDATA" "$V72_REPO" 750
...
V71_DEST="${KEMPNER_CACHE}/huggingface/lerobot/${V71_REPO}"
V72_DEST="${KEMPNER_CACHE}/huggingface/lerobot/${V72_REPO}"

python3 -c "
import json, pathlib
for name, path in [('v71', '${V71_DEST}'), ('v72', '${V72_DEST}')]:
...
echo "  bash r-preference/0331-v71-training-finetune-version2.sh"
echo "  bash r-preference/0331-v72-training-finetune-version2.sh"
echo ""
