#!/usr/bin/env python3
"""
RTC (Real-Time Chunking) closed-loop control with discrete diffusion — v2.

v2 fixes the inference/execution cycle timing (see Inferencer docstring).
...
Architecture:
  ...
  Inferencer thread:   Triggers at inference_delay into each execution window,
                        runs predict_action_realtime with prev chunk as prefix,
                        waits for execution_horizon to complete, then pushes
                        new_chunk[n_actions:] as the next execution window.
  Main thread:         Logging, gripper trick post-processing, early-stop.

vs v1 (closedloop_rtc.py):
  - No redundant first inference (v1 fired immediately, duplicating init)
  - Inference triggers at inference_delay (not execution_horizon)
  - Prev action chunk is the full current chunk (no ad-hoc shifting)
  - Free actions are new_chunk[n_actions:] (not new_chunk[actual_delay:])
  - Waypoints pushed only after execution window completes (no overlap)
  - Current chunk shifted by n_actions after each cycle for correct prefix

Robot WILL move. Use Ctrl+C to stop.

Usage:
    python ur5n/dd/closedloop_rtc_v2.py
    python ur5n/dd/closedloop_rtc_v2.py --n_actions 8 --inference_delay 4
    python ur5n/dd/closedloop_rtc_v2.py --instruction "pick up the block"
"""
...
class Inferencer:
    """Cadence-aware RTC inference worker — v2 (correct cycle).

    Cycle (chunk_len=16, n_actions=8, inference_delay=4):

      Init (main): predict_action → full 16-action chunk, push [0:8] to servo.

      Cycle 1:
        - After 4 actions consumed (inference_delay) → trigger inference.
          prev_action_chunk = current_normalized (full chunk, no shift).
        - Model returns new 16-action chunk; under hard_mask first 4 are
          copied from prev_action_chunk.
        - Wait for execution_horizon (8) to complete.
        - Push new_chunk[8:16] (8 free actions) to servo.
        - Shift: current_normalized ← [new_chunk[8:16], zeros[8:16]].

      Cycle 2+: same pattern — trigger at inference_delay into each
        execution window, push new_chunk[n_actions:] after window completes.
    """
    ...
    def _loop(self):
        ...
        # Init pushed [0:n_actions] starting at action_t=0.
        exec_start_t = 0

        while self._running:
            # ── 1. Wait for inference_delay actions consumed ──────────
            trigger_t = exec_start_t + self._inference_delay
            ...
            prev_norm_save = current_normalized.copy()
            ...
            prev_norm_batch = current_normalized[np.newaxis, ...]
            ...
            # ── 6. Push free actions: new_chunk[n_actions:] ──────────
            free_start = self._n_actions
            free_actions = new_actions_10d[free_start:]
            ...
            # ── 7. Update for next cycle: shift by execution_horizon ─
            # Current action chunk becomes new_chunk[n_actions:] (the
            # actions we just pushed), zero-padded to chunk_len.
            remaining = self._chunk_len - self._n_actions
            shifted = np.zeros_like(new_normalized)
            if remaining > 0:
                shifted[:remaining] = new_normalized[self._n_actions:]
            current_normalized = shifted
...
def main():
    ...
    # ═════════════════════════════════════════════════════════════════
    #  Start servo, then inferencer (waits for inference_delay consumed)
    # ═════════════════════════════════════════════════════════════════
    ...
    inferencer.start(current_normalized)
