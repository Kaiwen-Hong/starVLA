#!/usr/bin/env python3
"""
Air-hockey synchronous closed-loop control (Discrete Diffusion, 2-D world XY).

Blocking sync loop:
  1. Read current TCP pose (XY world)
  2. Grab wrist frame
  3. predict_action → (32, 2) delta (dx, dy) in world frame
  4. Take first n_actions; cumsum on current XY → n absolute XY waypoints
  5. Z and rotation locked at startup; pad each XY waypoint to 6-D
  6. Interpolate policy rate (50 Hz) → servo rate (100 Hz)
  7. servoL each waypoint blocking until executed, then re-loop

The robot WILL move. Press Ctrl+C to stop.

Usage:
    # Terminal 1: start the inference server (model loaded once)
    python ur5n/airhockey-d/dd/inference_server.py

    # Terminal 2: run the closed loop (defaults: --use_server, n_actions=16)
    python ur5n/airhockey-d/dd/closedloop_sync.py
    python ur5n/airhockey-d/dd/closedloop_sync.py --n_actions 8
"""
...
# ── Defaults ────────────────────────────────────────────────────────
DEFAULT_CHECKPOINT = (
    "checkpoints/discreteRTC/fastumi_airhockey_bounce_qwenDiscreteDiffusion_0523/"
    "checkpoints/steps_14000_pytorch_model.pt"
)
DEFAULT_INSTRUCTION = "strike the red puck back when it comes"
DEFAULT_SOCKET = "/tmp/starvla_infer_airhockey_dd.sock"

# Discrete-diffusion sampling defaults — match 2dd/closedloop_sync.py.
...
# Same constants as closedloop_rtc.py; we now want the option to moveJ
# the left arm into this distribution before starting the sync loop, so
# the locked Z + rotvec we read from getActualTCPPose() are guaranteed to
# match what the policy saw during training.
...
def sample_left_init_xy_with_ik(rtde_c, rtde_r, T_bw, seed=None):
    ...
    Verbatim copy of the function in closedloop_rtc.py — keep the two in
    sync if you tweak the home distribution.
    """
    import random
    if seed is not None:
        random.seed(seed)
...
def move_to_init(rtde_c, target_world_xyz, T_bw):
    """Blocking moveJ to the sampled init pose.

    Verbatim copy of the function in closedloop_rtc.py. Note that the
    second IK call here is NOT given a qnear seed — the solver picks its
    default joint solution. That's fine because we just IK-checked the
    same Cartesian target inside sample_left_init_xy_with_ik() using the
    current joints as qnear, so we know a reachable solution exists.
    """
    target_pose_base = [
...
def visualize_step(camera_image, current_xy, pred_delta, waypoints_xy,
    ...
    Solid red = executed waypoints (n_exec).
    Dashed pink = remainder of predicted chunk (not executed this step).
    """
    ...
    fig.suptitle(
        f'Sync closed-loop (DD) — step {step}\n'
...
def main():
    parser = argparse.ArgumentParser(
        description="Air-hockey sync closed-loop (Discrete Diffusion, 2-D world XY)")
    ...
    # Init-pose handling — added to mirror closedloop_rtc.py. Default
    # 'sample' will moveJ the LEFT arm into the training-distribution home
    # before locking Z + rotvec from getActualTCPPose(). Use 'current' to
    # skip the moveJ (legacy sync behavior — locks whatever you jogged to).
    ...
    # ── Discrete-diffusion sampling controls (mirrors 2dd/closedloop_sync.py)
    parser.add_argument("--decode_temperature", type=float,
                        default=DECODE_TEMPERATURE)
    parser.add_argument("--choice_temperature", type=float,
                        default=CHOICE_TEMPERATURE)
    ...
    # Discrete-diffusion sampling kwargs — match 2dd/closedloop_sync.py
    # (sync uses only the three temperature/argmax knobs; the RTC variant
    # additionally plumbs execution_horizon / fixed_steps / hard_mask /
    # early_stop, which are not meaningful for one-shot sync sampling).
    infer_kwargs = dict(
    ...
    # Mirrors closedloop_rtc.py. The sync loop locks Z + rotvec from
    # whatever pose it reads next, so moving into the training-distribution
    # home FIRST guarantees the locked values match what the policy saw
    # during data collection (Z=LEFT_HOME_DYNAMIC_Z=0.176, rotvec=
    # LEFT_HOME_DYNAMIC_ROTVEC). With --init_pose current we skip the
    # moveJ — useful for jog testing or if the arm is already where you want.
    ...
    # If --init_pose sample, this reads the achieved post-moveJ pose, so
    # Z ≈ LEFT_HOME_DYNAMIC_Z and rotvec ≈ LEFT_HOME_DYNAMIC_ROTVEC. With
    # --init_pose current, this is whatever the operator jogged to.
    ...
    # ── Print config + arm ─────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print(f"  Air-hockey Sync Closed-Loop (Discrete Diffusion)")
    ...
    print(f"  Decode temp:   {args.decode_temperature}  "
          f"Choice temp: {args.choice_temperature}  "
          f"simple_max: {args.use_simple_max}")
    ...
    try:
        while args.max_steps == 0 or step < args.max_steps:
            ...
            # Construct the full 6-D start pose for interpolation:
            # actual XY, locked Z + rot.
            current_6d = np.array(
            ...
            # ── Action-dim guard ──────────────────────────────────────
            # This script is hard-wired to 2-D (dx, dy) world-frame deltas:
            # cumsum builds XY waypoints, Z + rotvec are locked at startup,
            # there is no gripper channel. If the checkpoint accidentally
            # outputs 10-D fastumi actions (3 pos + 6D rot + grip) the cumsum
            # below would silently treat the rot/grip columns as XY deltas
            # and drive the arm out of the safety box. Fail loud here.
            assert pred_norm.shape == (chunk_len, 2), (
                f"Expected (chunk_len={chunk_len}, 2) air-hockey 2-D XY "
                f"actions, got {pred_norm.shape}. Did you point --checkpoint "
                f"at a 10-D fastumi pick-and-place model by mistake?")
