#!/usr/bin/env python3
"""
StarVLA inference server — air-hockey (Discrete Diffusion, 2-D world XY).
...
Avoids the ~25s model-load cost on every iteration of client-side code:
keep the server running in one terminal, edit and re-run the eval
script in another. The model stays warm in GPU memory across many
client runs.

Default socket is /tmp/starvla_infer_airhockey_dd.sock so this server
can run side by side with the airhockey-d/fm and 2dd/2fm servers
(which use other sockets).

Usage:
    # Terminal 1 — start the server (load once, leave running)
    python ur5n/airhockey-d/dd/inference_server.py
    # optional: --checkpoint <path>  --socket <unix-socket-path>

    # Terminal 2 — run a closed-loop script with --use_server (default)
...
def load_model(checkpoint_path):
    ...
    # Training config had image_size unset (=> no resize, native 256x256 from
    # the dataset went straight into Qwen-VL). Mirror that at eval time by
    # resizing PIL images to 256x256 — handles live camera (1080x1080 crop).
    if not getattr(config.datasets.vla_data, "image_size", None):
        config.datasets.vla_data.image_size = [256, 256]
        print("[server] [FIX] Set image_size=[256,256] (match training native)")
...
def main():
    parser = argparse.ArgumentParser(
        description="StarVLA inference server — air-hockey discrete-diffusion variant")
    ...
    parser.add_argument("--injected_delay", type=int, default=0,
                        help="Artificial delay (ms) added after each inference "
                             ...
                             "models for RTC timing tests. Default: 0")
