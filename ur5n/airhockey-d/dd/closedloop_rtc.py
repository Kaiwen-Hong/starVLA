#!/usr/bin/env python3
"""
RTC (Real-Time Chunking) closed-loop control for air-hockey
(Discrete Diffusion, 2-D world XY).

Three-thread architecture (mirrors ur5n/airhockey-d/fm/closedloop_rtc.py but
swaps the flow-matching head for the discrete-diffusion head; the robot-side
logic is identical):
  ...
  Inferencer thread:   wait_for_action_t(n_consumed) -> grab wrist frame ->
                       ...
                       <DD kwargs>) -> push output[inference_delay :
                       inference_delay+n_actions] into the servo buffer. Next
                       prev = output[n_actions:].
...
Usage:
    ...
    # n_actions=16, inference_delay=9)
    python ur5n/airhockey-d/dd/closedloop_rtc.py
    python ur5n/airhockey-d/dd/closedloop_rtc.py --n_actions 8 --inference_delay 4
    python ur5n/airhockey-d/dd/closedloop_rtc.py --save_rollout
"""
...
# Discrete-diffusion sampling defaults — match 2dd/closedloop_rtc_v6.py.
...
# The x_min carries an extra -0.1 m extension (mirrors airhockey-d/fm/
# closedloop_rtc.py): RTC sometimes asks the policy to splice tail poses
# that land slightly outside the strict training range and a tighter clamp
# would jerk the arm during chunk transitions.
...
def visualize_rtc_debug(result, pose_world, step, save_path, instruction,
    ...
    fig.suptitle(f"RTC Debug (DD) — Step {step}",
...
def visualize_rtc_summary(consistency_log, infer_times, pose_log, save_path,
    ...
    fig.suptitle("RTC Air-Hockey Rollout Summary (DD)",
...
def main():
    parser = argparse.ArgumentParser(
        description="RTC closed-loop air-hockey (Discrete Diffusion, 2-D XY)")
    ...
    # ── Discrete-diffusion sampling controls (mirrors 2dd/closedloop_rtc_v6.py)
    ...
    parser.add_argument("--no_hard_mask", dest="hard_mask",
                        action="store_false")
    ...
    # Discrete-diffusion sampling kwargs — match 2dd/closedloop_rtc_v6.py.
    # execution_horizon ties the DD sampler's per-step token budget to the
    # number of actions we actually push, so the head doesn't waste
    # iterations refining tokens that will be discarded.
    infer_kwargs = dict(
    ...
    # ── Print config ────────────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print(f"  Air-hockey RTC Closed-Loop (Discrete Diffusion)")
    ...
    print(f"  fixed_steps:      {args.fixed_steps}  "
          f"hard_mask: {args.hard_mask}  early_stop: {args.early_stop}")
    ...
    # cumsum (compute_waypoints) builds XY waypoints, Z + rotvec are locked
    # at startup, and there is no gripper channel. If the checkpoint
    # accidentally outputs 10-D fastumi actions (3 pos + 6D rot + grip)
    # every downstream array (deltas_xy, prev_action_chunk, waypoints,
    # visualization) would silently misinterpret the rot/grip columns as
    # XY deltas and drive the arm out of the safety box. Catch it at the
    # init inference here — both the ServoRunner and Inferencer threads
    # inherit this shape contract, so checking once is enough.
    assert current_normalized.shape == (chunk_len, 2), (
        f"Expected (chunk_len={chunk_len}, 2) air-hockey 2-D XY actions, "
        f"got {current_normalized.shape}. Did you point --checkpoint at a "
        f"10-D fastumi pick-and-place model by mistake?")
