...
The whole point: avoid the ~25s model-load cost when iterating on
client scripts. Run inference_server.py once in another terminal; this
client connects, fetches metadata (chunk_len, norm_stats, checkpoint
path), and proxies inference calls.

Used by ur5n/airhockey-d/fm/{replay,live}_openloop_eval.py when
--use_server is enabled. Default socket is
/tmp/starvla_infer_airhockey.sock so it does not collide with the
DD/fm servers running on other sockets.
"""
    ...
    def __init__(self, socket_path):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"  Start it first:  "
                f"python ur5n/airhockey-d/fm/inference_server.py"
            ) from e
