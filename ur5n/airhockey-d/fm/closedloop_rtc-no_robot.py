#!/usr/bin/env python3
"""
RTC (Real-Time Chunking) closed-loop control — NO-ROBOT variant
(air-hockey, QwenPI flow-matching, 2-D world XY).

Identical architecture to closedloop_rtc.py (three-thread: ServoRunner,
Inferencer, main) except the robot does NOT move:
  - ServoRunner still pops from the buffer, advances `action_t`, and
    updates `_last_pose` / `tail_pose` (so the Inferencer's wait_for_action_t
    and tail-splicing logic both work). It just NEVER calls servoL.
  - moveJ at init is skipped (init_pose=sample still samples + IK-checks
    for logging, but the robot is not jogged to the sampled XY).
  - servoStop / stopScript are skipped at cleanup.
  - RTDEControlInterface is kept only because the IK check used by
    sample_left_init_xy_with_ik lives on it.

Use this for:
  - Validating the RTC pipeline (consistency MAE, timing, viz) without
    putting the robot in motion.
  - Bench-testing inference cadence and starvation behavior.
...
Usage:
    ...
    # Terminal 2: run RTC no-robot loop
    python ur5n/airhockey-d/fm/closedloop_rtc-no_robot.py
    python ur5n/airhockey-d/fm/closedloop_rtc-no_robot.py --n_actions 4 --inference_delay 4
    python ur5n/airhockey-d/fm/closedloop_rtc-no_robot.py --save_rollout
"""
...
# ═══════════════════════════════════════════════════════════════════
#  ServoRunner — NO-ROBOT variant
#  Pops the buffer + advances action_t / tail_pose like normal, but
#  never issues servoL commands. The Inferencer thread sees identical
#  cadence (wait_for_action_t still unblocks correctly).
# ═══════════════════════════════════════════════════════════════════

class ServoRunner:
    """100 Hz virtual servo: pure bookkeeping, no robot commands."""

    def __init__(self, xy_lo, xy_hi, locked_z, locked_rot):
        self._xy_lo = xy_lo
        self._xy_hi = xy_hi
...
# ═══════════════════════════════════════════════════════════════════
#  Inferencer — cadence-aware RTC inference (unchanged from rtc.py)
# ═══════════════════════════════════════════════════════════════════
...
def visualize_rtc_debug(result, pose_world, step, save_path, instruction,
    ...
    ax_xy.plot(exec_traj[:, 0], exec_traj[:, 1], "s-",
               ...
               label=f"exec [{D}:{D + A}) ({result.n_pushed}) [NOT SENT]")
    ...
    fig.suptitle(f"RTC Debug [NO-ROBOT] — Step {step}",
...
def visualize_rtc_summary(consistency_log, infer_times, pose_log, save_path,
    ...
    fig.suptitle("RTC Air-Hockey Rollout Summary [NO-ROBOT]",
    ...
    ax.set_title("Robot XY over time (read-only)", fontweight="bold")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    if pose_log:
        ...
        ax.set_title("Top-Down Trajectory (read-only)", fontweight="bold")
...
def main():
    parser = argparse.ArgumentParser(
        description="RTC closed-loop air-hockey [NO-ROBOT] "
                    "(QwenPI flow-matching, 2-D XY)")
    ...
    parser.add_argument("--init_pose", choices=["sample", "current"],
                        ...
                        help="'sample' = IK-checked random from training "
                             "distribution (NO moveJ in no-robot mode); "
                             "'current' = use current pose as-is.")
    ...
    # ── Connect robot ───────────────────────────────────────────────
    # RTDEControlInterface is kept ONLY because sample_left_init_xy_with_ik
    # needs getInverseKinematics for the IK check. We never call moveJ /
    # servoL / servoStop / stopScript.
    from rtde_control import RTDEControlInterface
    from rtde_receive import RTDEReceiveInterface
    robot_ip = ROBOT_IPS[args.arm]
    print(f"\nConnecting to {args.arm} arm at {robot_ip} (NO MOVEMENT)...")
    ...
    # ── Initial pose: sample (IK-check only) or use current ─────────
    if args.init_pose == "sample":
        ...
        print(f"  [NO-ROBOT] skipping moveJ — robot stays at current pose")

    # ── Lock Z + rotation from current (not jogged) pose ────────────
    ...
    if args.save_rollout:
        ...
        rollout_dir = (Path("ur5n/airhockey-d/fm/rollouts_rtc_no_robot")
                       / f"rtc_{ts_str}")
    ...
    # ── Print config ────────────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print(f"  Air-hockey RTC Closed-Loop [NO-ROBOT] (QwenPI flow-matching)")
    print(f"  Arm:              {args.arm}  IP={robot_ip}  (NO MOVEMENT)")
    ...
    # NO-ROBOT ServoRunner: no rtde_c, no servoL — pure bookkeeping.
    servo = ServoRunner(xy_lo, xy_hi, locked_z, locked_rot)
    ...
    try:
        ...
        try:
            ...
            pass
        # No servoStop / stopScript — we never issued movement commands.
        cam.close()
