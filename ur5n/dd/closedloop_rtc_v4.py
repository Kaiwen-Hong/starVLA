...
def main():
    ...
    # Push first (inference_delay + n_actions) into servo buffer so that
    # when n_actions are consumed, inference_delay actions remain — matching
    # the steady-state trigger condition for the first cycle.
    n_init = min(args.inference_delay + args.n_actions, len(current_actions_10d))
    ...
    init_prev_chunk = current_normalized[args.n_actions:]
