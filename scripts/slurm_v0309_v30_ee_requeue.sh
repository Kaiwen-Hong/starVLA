...
# ============================================================
# StarVLA Custom v0309 v30 EE (16D end-effector) on kempner_requeue
# 1 node x 4 H200 80GB, DeepSpeed ZeRO-2, QwenPI
#
# Key differences from joint-space Custom scripts:
#   - action_dim = 16 (not 14): 3 pos + 4 quat + 1 gripper per arm
#   - state_dim = 16 (not 14): same layout
#   - data_root_dir = playground/Datasets/CustomEE (separate from 14D)
#   - data_mix = custom_v0309_v30_ee
#   - framework = QwenPI (not QwenOFT)
#   - base_vlm = Qwen2.5-VL-3B-Instruct-Action
#
...
accelerate launch \
  ...
  --run_id v0309_v30_ee_qwenPI_requeue \
  --wandb_project starVLA_v30 \
  --wandb_entity hca
