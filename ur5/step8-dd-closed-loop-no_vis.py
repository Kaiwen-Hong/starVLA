#!/usr/bin/env python3
"""
Step 8: Closed-loop control with real robot (Discrete Diffusion) — no visualization.

Stripped-down version of step6-dd for maximum speed:
  No matplotlib, no image saving, no rollout logging, no trajectory computation.
...
Synchronous receding-horizon loop:
    ...
    4. Denormalize
    5. Execute first N actions via servoL
    6. Repeat from step 1

The robot WILL move. Use Ctrl+C to stop at any time.

Usage:
    python ur5/step8-dd-closed-loop-no_vis.py
    python ur5/step8-dd-closed-loop-no_vis.py --n_actions 14 --arm left
    python ur5/step8-dd-closed-loop-no_vis.py --use_simple_max
    python ur5/step8-dd-closed-loop-no_vis.py --decode_temperature 0.5
"""
...
# ═══════════════════════════════════════════════════════════════════
#  Main closed-loop (no visualization)
# ═══════════════════════════════════════════════════════════════════

def main():
    parser = argparse.ArgumentParser(
        description="Closed-loop control with UR5 (Discrete Diffusion, no vis)")
    parser.add_argument(
        "--checkpoint", type=str,
    ...
    # ── Print config ─────────────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print(f"  Closed-loop control (DD, no vis)")
