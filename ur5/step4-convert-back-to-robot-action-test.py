...
The model outputs relative actions in 10D LeRobot format:
    [rel_x, rel_y, rel_z, rot6d(6), gripper]

This script converts them to 7D UR5-readable format:
    [rel_x, rel_y, rel_z, rel_rx, rel_ry, rel_rz, gripper]

Conversion (see docs/how-to-process-to-lerobotv21-format.md Section 5a):
    Model output: normalized relative action (T, 10)
        ↓ denormalize (norm_stats from checkpoint)
    Denormalized action (10D): [rel_xyz(3), rel_rot6d(6), gripper(1)]
        ↓ rot6d → rotation matrix (Gram-Schmidt)
        ↓ rotation matrix → axis-angle (scipy Rotation.as_rotvec)
    Relative action (7D): [rel_x, rel_y, rel_z, rel_rx, rel_ry, rel_rz, gripper]

Usage:
    python ur5/step4-convert-back-to-robot-action-test.py
    python ur5/step4-convert-back-to-robot-action-test.py --include_state
    python ur5/step4-convert-back-to-robot-action-test.py --checkpoint <path>
"""
...
def rot6d_to_axisangle(d6: np.ndarray) -> np.ndarray:
    """rot6d (6,) → axis-angle (3,) via Gram-Schmidt then as_rotvec.
    See docs/how-to-process-to-lerobotv21-format.md Section 5a."""
...
def actions_10d_to_7d(actions_10d: np.ndarray) -> np.ndarray:
    """
    Convert (T, 10) relative actions from LeRobot format to UR5 format.

    Input  per step: [rel_x, rel_y, rel_z, rot6d(6), gripper]   (10D)
    Output per step: [rel_x, rel_y, rel_z, rel_rx, rel_ry, rel_rz, gripper]  (7D)
    """
    T = actions_10d.shape[0]
    actions_7d = np.zeros((T, 7), dtype=np.float32)
    for t in range(T):
        actions_7d[t, :3] = actions_10d[t, :3]                     # rel xyz
        actions_7d[t, 3:6] = rot6d_to_axisangle(actions_10d[t, 3:9])  # rot6d → axis-angle
        actions_7d[t, 6] = actions_10d[t, 9]                       # gripper
    return actions_7d
...
def ee_pose_to_state10d(pose_6d: list, gripper: float) -> np.ndarray:
    """[x, y, z, rx, ry, rz] + gripper → 10D state [xyz, rot6d(6), gripper]."""
...
# ═══════════════════════════════════════════════════════════════════
#  Model loading & inference (shared with step3)
# ═══════════════════════════════════════════════════════════════════
...
def visualize(actions_7d: np.ndarray, camera_image: Image.Image, save_path: str):
    """
    Visualize predicted relative actions (7D, denormalized).

    Layout: camera image (left), rel_x / rel_y / rel_z / gripper (right).
    """
    T = actions_7d.shape[0]
    ts = np.arange(T) / 20.0  # 20 Hz
    ...
    # Right: rel_x, rel_y, rel_z, gripper
    dims = [
        (0, "rel_x (m)", "#e41a1c"),
        (1, "rel_y (m)", "#377eb8"),
        (2, "rel_z (m)", "#4daf4a"),
    ...
    fig.suptitle(
        f'Step 4: Denormalized Relative Actions\n"{INSTRUCTION}"',
...
def main():
    parser = argparse.ArgumentParser(
        description="Denormalize model predictions and convert to UR5 format (test only)")
    parser.add_argument(
        "--checkpoint", type=str,
    ...
    try:
        while True:
            ...
            # Diagnostic: show raw model output for gripper dim before denorm
            ...
            save_path = Path("ur5") / f"step4_actions_viz_{timestamp}.png"
