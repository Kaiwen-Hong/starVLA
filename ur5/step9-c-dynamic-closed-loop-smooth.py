#!/usr/bin/env python3
"""
Step 9c-dynamic: Closed-loop control with smoother motion (QwenPI / flow-matching)
for the dynamic pick-and-place task (turntable).

Same smooth-motion approach as step9-c but with the dynamic-324 checkpoint:
...
Usage:
    python ur5/step9-c-dynamic-closed-loop-smooth.py
    python ur5/step9-c-dynamic-closed-loop-smooth.py --n_actions 14 --arm left
    python ur5/step9-c-dynamic-closed-loop-smooth.py --include_state
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Closed-loop control with smooth interpolated servoL "
                    "(QwenPI / flow-matching, dynamic pick-and-place)")
    parser.add_argument(
        "--checkpoint", type=str,
    ...
    parser.add_argument("--camera_dev", type=int, default=1)
    ...
    # ── Print config ─────────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print(f"  Step 9c-dynamic: Closed-loop (QwenPI FM, smooth {SERVO_HZ}Hz)")
    ...
    if args.save_rollout:
        ...
        rollout_dir = Path(args.rollout_dir) if args.rollout_dir else (
            Path("ur5") / "rollouts" / f"step9c_dynamic_fm_{ts}")
        run_config = {
            "mode": "sync", "method": "step9c-dynamic-fm",
