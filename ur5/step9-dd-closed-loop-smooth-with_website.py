#!/usr/bin/env python3
"""
Step 9 + Website Stopwatch: Closed-loop control with smooth interpolation (sync).

Same as step9-dd-closed-loop-smooth.py but with:
...
def visualize_step(pred_poses, camera_image, current_ee,
    ...
    for row, (dim_idx, label, color, start_val) in enumerate(dims):
        ...
        # Tail beyond n_exec (dotted, faded)
        if n_exec < T_pred:
    ...
    fig.suptitle(
        f'Step {step_idx} (DD-Sync-Smooth): exec={n_exec}\n'
        f'"{instruction}"',
...
def main():
    parser = argparse.ArgumentParser(
        description="Closed-loop control (sync) with smooth interpolated servoL + website stopwatch")
    parser.add_argument(
        "--checkpoint", type=str,
    ...
    # -- Print config --
    print(f"\n{'=' * 60}")
    print(f"  Step 9: Closed-loop SYNC (DD, smooth {SERVO_HZ}Hz)")
    ...
    if args.save_rollout:
        if args.rollout_dir:
            ...
            rollout_dir = Path("ur5") / "rollouts" / f"dd_sync_smooth_{ts}"
