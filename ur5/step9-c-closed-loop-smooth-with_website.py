#!/usr/bin/env python3
"""
Step 9c + Website Stopwatch: Closed-loop control with smooth interpolation
(Continuous / QwenPI, sync).

Same as step9-c-closed-loop-smooth.py but with:
  1. An embedded HTTP server with auto-triggered browser stopwatch
  2. Early stop when grasped + lifted high enough
  3. Detailed per-step timing breakdown (obs / action_gen / send)
  4. End-of-run timing summary
  5. Optional rollout saving (JSON + viz images)

This is the SYNCHRONOUS version: observe -> infer -> execute, no overlap.
...
def visualize_step(pred_poses, camera_image, current_ee,
                   n_exec, step_idx, save_path, instruction):
    """Visualize the predicted trajectory for the current step."""
    T_pred = pred_poses.shape[0]
    ...
    for row, (dim_idx, label, color, start_val) in enumerate(dims):
        ...
        pred_vals = pred_poses[:, dim_idx]
        ax.plot(ts_pred[:n_exec], pred_vals[:n_exec], "o-",
                markersize=5, linewidth=2.0, color=color,
                label="executed" if row == 0 else None)
        if n_exec < T_pred:
            ax.plot(ts_pred[n_exec - 1:], pred_vals[n_exec - 1:], "o:",
        ...
        ax.axvline(ts_pred[n_exec - 1], color="black", linewidth=1.0,
                   linestyle=":", alpha=0.6,
    ...
    fig.suptitle(
        f'Step {step_idx} (Continuous-Sync-Smooth): exec={n_exec}\n'
        f'"{instruction}"',
...
def main():
    parser = argparse.ArgumentParser(
        description="Closed-loop control (sync, Continuous/QwenPI) with smooth servoL + website stopwatch")
    parser.add_argument(
        "--checkpoint", type=str,
    ...
    # -- Print config --
    print(f"\n{'=' * 60}")
    print(f"  Step 9c: Closed-loop SYNC (Continuous/QwenPI, smooth {SERVO_HZ}Hz)")
    ...
    if args.save_rollout:
        if args.rollout_dir:
            ...
            rollout_dir = Path("ur5") / "rollouts" / f"c_sync_smooth_{ts}"
    ...
    stopwatch.start_timer(splash_title="Flow Matching\nSync Policy")
    print("[STOPWATCH] Started")

    # -- Control loop (synchronous: observe -> infer -> execute) --
    step = 0
    ...
    try:
        while args.max_steps == 0 or step < args.max_steps:
            ...
            # 1. Read current robot state + grab camera (observation)
            ...
            # 3. Inference (synchronous, continuous / flow matching — no temperature args)
            ...
            # 4. Convert to absolute world-frame waypoints + execute
            ...
            deltas_3d = np.array([action_10d_to_delta7d(pred_actions_10d[i])[:3]
                                  for i in range(n_exec)])
            ...
            if args.save_rollout and rollout_dir is not None:
                exec_deltas_7d = actions_10d_to_7d(pred_actions_10d)
                pred_poses_all = accumulate_deltas(pose_world, exec_deltas_7d)

                viz_path = str(rollout_dir / "images" / f"step_{step:04d}_viz.png")
                viz_executor.submit(
                    visualize_step,
                    pred_poses_all.copy(), pil_img.copy(), list(pose_world),
                ...
                step_data = {
                    ...
                    "pred_trajectory_world": pred_poses_all.tolist(),
