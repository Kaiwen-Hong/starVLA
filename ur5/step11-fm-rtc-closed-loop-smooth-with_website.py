#!/usr/bin/env python3
"""
Step 11 + Website Stopwatch: Closed-loop control with Flow-Matching RTC +
smooth interpolation and an auto-triggered browser stopwatch.

Same as step11-fm-rtc-closed-loop-smooth.py but with:
  1. An embedded HTTP server with auto-triggered browser stopwatch
  2. Camera capture moved into the async inference thread
  3. Detailed per-step timing breakdown (obs / action_gen / send)
  4. End-of-run timing summary with Hz and extra-wait stats

Usage:
    python ur5/step11-fm-rtc-closed-loop-smooth-with_website.py
    python ur5/step11-fm-rtc-closed-loop-smooth-with_website.py --n_actions 14 --arm left
    python ur5/step11-fm-rtc-closed-loop-smooth-with_website.py --n_actions 8 --inference_delay 8 --arm left --no_save_rollout
    python ur5/step11-fm-rtc-closed-loop-smooth-with_website.py --stopwatch_port 8765
"""
...
class AsyncInference:
    ...
    def start(self, model, cam, instruction, state_10d,
              prev_normalized, inference_delay):
...
def main():
    ...
    try:
        while args.max_steps == 0 or step < args.max_steps:
            ...
            # 3. Launch background camera capture + inference (FM-RTC)
            state_for_model = current_state_10d if args.include_state else None
            ...
            # 6. Wait for background camera+inference to complete
            extra_wait_ms = 0.0
