#!/usr/bin/env python3
"""
Step 50: Open-loop dataset inference with visualization.

Runs QwenPI predict_action on dataset samples and saves per-sample
visualizations (image + predicted vs GT normalized actions) for
side-by-side comparison with live inference (step5/step9).

Usage:
    python ur5/step50-fm-openloop-dataset-inference-viz.py
    python ur5/step50-fm-openloop-dataset-inference-viz.py --num_samples 50
    python ur5/step50-fm-openloop-dataset-inference-viz.py --no_only_first_half
    python ur5/step50-fm-openloop-dataset-inference-viz.py --randomly_sample
"""
...
def visualize_sample(image, pred, gt, sample_idx, chunk_len,
                     instruction, save_path, mse, l1):
    """Visualize one sample: image (left) + pred vs GT (right).

    Layout (right side):
      Row 0: pos_x        (full width)
      Row 1: pos_y        (full width)
      Row 2: pos_z        (full width)
      Row 3: rot6d 0-5    (6 small subplots, 2 rows x 3 cols)
      Row 4: (cont.)
      Row 5: gripper      (full width)

    pred, gt: (T, 10) normalized actions.
    """
    fig = plt.figure(figsize=(16, 16))
    gs_main = GridSpec(1, 2, figure=fig, width_ratios=[1, 1.3], wspace=0.25)
    ...
    def _plot_dim(ax, dim_idx, show_xlabel=False):
        color = DIM_COLORS[dim_idx]
        label = DIM_LABELS[dim_idx]
        ax.plot(ts, pred[:, dim_idx], "o-", markersize=3, linewidth=1.5,
                color=color, label=f"pred")
        ax.plot(ts, gt[:, dim_idx], "x--", markersize=3, linewidth=1.2,
                color=color, alpha=0.6, label=f"GT")
    ...
    # Rotation: 2 rows x 3 cols of small subplots
    for rot_row in range(2):
        gs_rot = GridSpecFromSubplotSpec(
            1, 3, subplot_spec=gs_right[3 + rot_row], wspace=0.35)
        for col in range(3):
            dim_idx = 3 + rot_row * 3 + col
            ax = fig.add_subplot(gs_rot[0, col])
            color = DIM_COLORS[dim_idx]
            ax.plot(ts, pred[:, dim_idx], "o-", markersize=2, linewidth=1.2,
                    color=color)
            ax.plot(ts, gt[:, dim_idx], "x--", markersize=2, linewidth=1.0,
    ...
    # Gripper
    ax = fig.add_subplot(gs_right[5])
    _plot_dim(ax, 9, show_xlabel=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Open-loop dataset inference with visualization (QwenPI)")
    ...
    rollout_dir = Path(args.rollout_dir) if args.rollout_dir else (
        Path("ur5") / "rollouts" / f"step50_fm_openloop_{ts}")
    ...
    for count, ds_idx in enumerate(tqdm(indices[:num_samples], desc="Evaluating")):
        ...
        dataset_image = sample["image"][0]  # PIL Image (224x224)

        # Save visualization only (no raw image)
        viz_path = rollout_dir / "images" / f"sample_{count:04d}_viz.png"
        visualize_sample(dataset_image, pred, gt, count, chunk_len,
                         args.instruction, str(viz_path), mse, l1)
    ...
    print(f"\n{'=' * 60}")
    print(f"  Open-Loop Dataset Inference (QwenPI FM)")
    print(f"{'=' * 60}")
