...
class LayerwiseDiscreteDiffusionActionHead(nn.Module):
    ...
    def __init__(self, global_config, action_norm_stats=None):
        ...
        DiTConfig["num_attention_heads"] = (
            DiTConfig["input_embedding_dim"] // DiTConfig["attention_head_dim"]
        )
        ...
        self.inner_dim = (
            DiTConfig["num_attention_heads"] * DiTConfig["attention_head_dim"]
        )
        ...
        if action_config.get("state_dim", 0) > 0:
            self.state_encoder = MLP(
                action_config.state_dim, self.inner_dim, self.inner_dim
            )
        else:
            self.state_encoder = None

        self.future_tokens = nn.Embedding(
            action_config.get("num_target_vision_tokens", 32), self.inner_dim
        )
        nn.init.normal_(self.future_tokens.weight, mean=0.0, std=0.02)

        if action_config.get("add_pos_embed", True):
            self.position_embedding = nn.Embedding(
                action_config.get("max_seq_len", 1024), self.inner_dim
            )
    ...
    def apply_mask(self, input_tokens, generator=None):
        ...
        mask_ratios = train_mask_schedule(
            torch.rand(B, device=device, generator=generator), self.train_mask_schedule
        )
        num_mask = (total_unknown * mask_ratios).round().long().clamp(1, L)
        num_mask = torch.where(
            total_unknown > 0, num_mask, torch.zeros_like(num_mask)
        )
    ...
    def _forward_logits(self, vl_embs_list, token_ids, state_features, device):
        ...
        temb = self.model.timestep_encoder(
        ...
        sa_embs = (
            self.model.norm_out(sa_embs) * (1 + scale[:, None]) + shift[:, None]
        )
        ...
        return action_logits.reshape(
            B, self.action_horizon, self.action_dim, self.binning.logits_dim
        )
    ...
    def forward(self, vl_embs_list, actions, state=None, logging: bool = False):
        ...
        logits = self._forward_logits(
            vl_embs_list, x, state_feat, actions.device
        )
        if logging:
    ...
    def loss(self, pred, target, loss_mask=None, actions=None, **kwargs):
        ...
        if loss_mask is None:
            loss_mask = torch.ones(
                B, *target.shape[1:], dtype=torch.bool, device=device
            )
        ...
        if self.binning.representation == "bin":
            ...
            bce = F.binary_cross_entropy_with_logits(
                pred_flat, bit_targets, reduction="none"
            ).mean(dim=-1)
            ce = bce
...
def get_action_model(config=None, action_norm_stats=None):
    return LayerwiseDiscreteDiffusionActionHead(
        global_config=config, action_norm_stats=action_norm_stats
    )
