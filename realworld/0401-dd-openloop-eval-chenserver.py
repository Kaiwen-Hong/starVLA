...
# ── Defaults ─────────────────────────────────────────────────────────
DEFAULT_CHECKPOINT = (
    "/scratch/wangpc/starVLA/results/Checkpoints/fastumi_pickandplace_qwenDiscreteDiffusion_329v3/"
...
def evaluate(model, dataloader, num_samples: int, infer_kwargs: dict,
             pos_scale: np.ndarray = None, train_batch_size: int = 64) -> dict:
    all_mse = []
    all_l1 = []
    ...
    n_padded_steps = 0
    n_total_steps = 0
    # Simulate training batch-level metric: buffer preds/gts in chunks of
    # train_batch_size, then compute L2-norm / N per chunk (exactly like training).
    batch_pred_buf = []    # list of [T, 10] arrays
    batch_gt_buf = []
    batch_mse_scores = []  # one score per simulated batch
    ...
    for batch in tqdm(dataloader, total=min(num_samples, len(dataloader)), desc="Evaluating"):
        ...
        # Buffer for batch-level metric (no masking, use full arrays like training)
        batch_pred_buf.append(pred)
        batch_gt_buf.append(gt)
        if len(batch_pred_buf) == train_batch_size:
            stacked_pred = np.stack(batch_pred_buf)  # [B, T, 10]
            stacked_gt = np.stack(batch_gt_buf)
            num_pots = np.prod(stacked_pred.shape)
            score = np.linalg.norm(stacked_pred - stacked_gt) / num_pots
            batch_mse_scores.append(score)
            batch_pred_buf = []
            batch_gt_buf = []

        # Per-sample metrics (for distribution plots)
        diff = pred_v - gt_v
        mse = np.mean((diff) ** 2)
        ...
        per_dim_mse = np.mean((diff) ** 2, axis=0)  # [10,]
        per_dim_l1 = np.mean(np.abs(diff), axis=0)
    ...
    # Flush remaining samples in buffer (partial batch)
    if batch_pred_buf:
        stacked_pred = np.stack(batch_pred_buf)
        stacked_gt = np.stack(batch_gt_buf)
        num_pots = np.prod(stacked_pred.shape)
        score = np.linalg.norm(stacked_pred - stacked_gt) / num_pots
        batch_mse_scores.append(score)
    ...
    # Batch-level mse_score: average across simulated batches of train_batch_size
    global_mse_score = float(np.mean(batch_mse_scores)) if batch_mse_scores else 0.0

    print(f"\n  [Padding] {n_padded_steps}/{n_total_steps} timesteps excluded "
          f"({100*n_padded_steps/max(n_total_steps,1):.1f}%)")
    print(f"  [Batch metric] {len(batch_mse_scores)} simulated batches of {train_batch_size}")
    ...
    results = {
        ...
        "global_mse_score": global_mse_score,
...
def print_results(results: dict, checkpoint_name: str = ""):
    ...
    print(f"  Global mse_score  : {results['global_mse_score']:.6f}  (training metric: L2_norm/N, batch_size={results.get('train_batch_size', 64)})")
...
def main():
    ...
    parser.add_argument("--train_batch_size", type=int, default=64,
                        help="Simulated training batch size for mse_score metric")
