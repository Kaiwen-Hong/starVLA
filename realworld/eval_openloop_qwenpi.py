#!/usr/bin/env python3
"""
Open-loop evaluation for FastUMI pick-and-place QwenPI policy.
Loads a trained QwenPI checkpoint, runs predict_action on the dataset,
...
def load_model(checkpoint_path: str):
    """
    Load a QwenPI model from checkpoint.
    Replicates baseframework.from_pretrained logic but auto-detects
    attn_implementation (flash_attention_2 vs sdpa) before building.
    """
    ...
    # Override attn_implementation based on availability
...
def load_dataset(model, include_state: bool = True):
    """
    Re-create the training dataset using model's saved config.
    include_state is injected because the checkpoint config.yaml may not
    have this field even though training used --include_state true.
    """
...
def evaluate(model, dataloader, num_samples: int) -> dict:
    ...
    chunk_len = model.chunk_len  # past_action_window_size + 1 + future_action_window_size = 16
