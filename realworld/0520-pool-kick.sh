#!/usr/bin/env bash
############# 0520 pool-pocket-wall KICK #############
# Mirrors 0520-dynamic-air-hockey-static-openpi.sh — same QwenPI recipe, swapped to the
# pool "kick" task (single wrist cam, FastUMI, 10D EEF + rot6d, action chunk 16).
#   task: "Strike the white ball off the cushion to hit the black ball"  (51 demos, 20fps)
#
# Default = all 8 H100. If GPUs 0,1 are busy, run on the 6 free ones instead:
#   CUDA_VISIBLE_DEVICES=2,3,4,5,6,7 NUM_GPUS=6 bash realworld/0520-pool-kick.sh
NUM_GPUS=${NUM_GPUS:-8}
...
accelerate launch \
  ...
  --datasets.vla_data.data_mix pool_pocket_wall_kick \
  ...
  --run_id fastumi_pool_kick_qwenPI_0520 \
  --wandb_project starVLA_pool_kick_0520 \
